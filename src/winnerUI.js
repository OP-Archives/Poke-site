...
export default function Winners(props) {
  ...
  const createMatches = async (evt) => {
    ...
    //check for winners to advance to next round
    for (let i = 0; i < tmpMatches.length; i++) {
      const match = tmpMatches[i];
      if (!match.winner_id) continue;
      const nextRoundMatches = tmpMatches.filter(
        (argMatch) => parseInt(argMatch.round) === parseInt(match.round) + 1
      );

      if (nextRoundMatches.length === 0) return setMatches(tmpMatches);

      const thisRoundMatches = tmpMatches.filter(
        (argMatch) => parseInt(argMatch.round) === parseInt(match.round)
      );

      let indexOfMatch = -1;
      for (let i = 0; i < thisRoundMatches.length; i++) {
        if (parseInt(thisRoundMatches[i].id) !== match.id) continue;
        indexOfMatch = i;
        break;
      }
